---
name: usage-report
description: Price local Codex CLI usage at OpenAI API rates. Reads rollout jsonl from one or more CODEX_HOME dirs, splits tokens by model and short/long context, rebuilds weekly quota cycles (0→100 %) per account, fetches current prices from the OpenAI pricing page, and renders a screenshot-ready HTML report.
---

# usage-report

Use when the user asks how many tokens Codex consumed, what that usage would cost at API prices,
...
- Old rollouts (before codex wrote `token_usage_record`) are counted from deltas of `token_count.total_token_usage`; both methods agree where both exist.
