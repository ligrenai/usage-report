#!/usr/bin/env python3
"""Collect provider token usage from local files and price it at API rates -> out/usage.json

  daily  : per local day, per account, per model, short/long context split, API-equivalent USD
  cycles : per account quota cycles (one weekly used_percent window each), same splits + USD per 1 %
...
def collect(args):
    ...
    prices = load_prices(args.prices); TH = prices['long_context_threshold']
    ...
    def add(bucket, model, cls, i, c, o):
        key = model if cls == 'short' else f'{model}[1m]'
        b = bucket.setdefault(key, {'model': model, 'context': cls, 'input': 0, 'cached': 0, 'output': 0, 'requests': 0, 'usd': 0.0, 'usd_standard': 0.0,
                                    'billable_uncached': 0.0, 'billable_cached': 0.0, 'billable_output': 0.0})
        b['input'] += i; b['cached'] += c; b['output'] += o; b['requests'] += 1; b['usd'] += cost(model, cls, i, c, o); b['usd_standard'] += cost(model, 'short', i, c, o)
        # billable tokens: long-context tokens weighted by (long price / short price) per token type, so token charts follow the pricing rule
        ps, pl = price(model, 'short'), price(model, cls)
        w = lambda f: (pl.get(f, 0) / ps.get(f)) if ps.get(f) else 1.0
        b['billable_uncached'] += (i - c) * w('input'); b['billable_cached'] += c * w('cached'); b['billable_output'] += o * w('output')
    ...
    for ts, acc, model, i, c, o, size in recs:
        d = (parse_ts(ts) + tz).strftime('%Y-%m-%d')
        if d < since or d > until or (acc in acc_from and d < acc_from[acc]): continue
        add(daily[d].setdefault(acc, {}), model, 'long' if size > TH else 'short', i, c, o)
        add(hourly[parse_ts(ts).strftime('%Y-%m-%dT%H:00Z')].setdefault(acc, {}), model, 'long' if size > TH else 'short', i, c, o)
    ...
    for acc, s in series.items():
        ...
        for gi, g in enumerate(gl):
            ...
            by = {}
            for ts, a, model, i, c, o, size in recs:
                if a == acc and t0 <= ts <= t1: add(by, model, 'long' if size > TH else 'short', i, c, o)
            tin = sum(v['input'] for v in by.values())
            if tin < args.min_tokens: continue
            tout = sum(v['output'] for v in by.values()); usd = sum(v['usd'] for v in by.values()); usd_std = sum(v['usd_standard'] for v in by.values())
            ...
            cycles.append({'account': acc, 'plan': plan, 'start': d0.strftime('%Y-%m-%d %H:%M'), 'end': d1.strftime('%Y-%m-%d %H:%M'),
                           ...
                           'usd': round(usd, 2), 'usd_per_pct': round(usd / max(consumed, 1), 2), 'usd_standard': round(usd_std, 2), 'usd_standard_per_pct': round(usd_std / max(consumed, 1), 2), 'by_model': by,
    ...
    for c in cycles:
        ...
        for k, v in c['by_model'].items(): share[v['model']] += v['input']
    ...
    def window_stats(cs):
        ...
        return {'windows': n, 'accounts': sorted({c['account'] for c in cs}), 'cycle_ids': [f"{c['account']} {c['start']}" for c in cs],
                ...
                'long_share_of_input': round(sum(v['input'] for c in cs for v in c['by_model'].values() if v['context'] == 'long') / max(sum(c['input'] for c in cs), 1), 3)}
    eras = {}
    if args.era_split:
...
def main():
    ...
    for c in out['cycles']:
        print(f"{c['account']:<6} {c['start'][5:]} -> {c['end'][5:]}  {c['hours']:>6}h  {c['start_pct']:>3.0f}->{c['peak_pct']:>3.0f}%  in {c['input']/1e9:5.2f}B out {c['output']/1e6:6.2f}M  ${c['usd']:>7,.0f}  ${c['usd_per_pct']:>5.1f}/1%  {'FULL' if c['complete'] else 'part'}")
